...
void FastSalience::copy(const FastSalience &rhs) {
	if (false) cout << "Calling copy" << endl; 
...
void FastSalience::updateSalience(const Mat &frame)  {
	...
	if (temporalImageI[0].cols != frame.cols || temporalImageI[0].rows != frame.rows) {
		for (size_t i = 0; i < temporalImageI.size(); i++) {
			if (false) cout << "For temporal scale "<< i << " resetting temporal images." << endl; 
	...
	for (int i=0; i<nspatial; i++) {
		if (false) cout << "For spatial scale "<< i << endl; 
		if (!useColor) {
			iDoB = redBoxConvolutionAtScale[i+1] - redBoxConvolutionAtScale[i]; 
		} else {
			if (false) cout << "Computing color contrasts" << endl; 
		...
		for (int j=0; j<ntemporal; j++) {
			if (false) cout << "Updating temporal response " << j << endl; 
		...
		if (useColor) {
			for (int j=0; j<ntemporal; j++) {
				if (false) cout << "Updating temporal response rg " << (i*ntemporal+j) << endl; 
				double a = tau.at<double>(0,j); 
				double b = 1.0/(1.0+a); // b = 1/(1+tau)
				a = a *b;               // a = tau / (1+tau)
			...
			for (int j=0; j<ntemporal; j++) {
				if (false) cout << "Updating temporal response by " << (i*ntemporal+j) << endl; 
				double a = tau.at<double>(0,j); 
				double b = 1.0/(1.0+a); // b = 1/(1+tau)
				a = a *b;               // a = tau / (1+tau)
		...
		if (useDoE) {
			...
			if (false) cout << "Computing DoE filters probs " << endl; 
			//update saliency
		...
		if (useDoB) {
			if (false) cout << "Probs of DoB" << endl; 
			Mat dummy; 
